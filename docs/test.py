...
def get(all_list = []):
    stock_symbol_list, price_low_list, price_high_list = manage(all_list)
    ...
    for i in range(int(len(all_list) / 3)):
        ...
        elif float(price_list[i]) >=  float(price_low_list[i]):
...
login()
push(['000001', 10, 11, '000002', 20, 21, '000004', 40, 41, '000005', 50, 51])
